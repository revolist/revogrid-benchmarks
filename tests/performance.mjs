...
async function testPage(page, url, scrollSelector, firstCellSelector) {
  ...
  try {
    ...
    fps = await page.evaluate(async (scrollSelector) => {
      ...
      let frameCount = 0;
      const scrollDuration = 5000; // Duration to scroll (in ms)
...
(async () => {
  ...
  const sets = [1000, 100000, 200000, 400000];
  for (let i = 0; i < sets.length; i++) {
    ...
    try {
      await testPage(
        ...
        '.ht_master.handsontable > .wtHolder td[role="gridcell"][aria-colindex="1"]'
      );
    } catch (error) {
    ...
    try {
      await testPage(
        await context.newPage(),
        "rv",
        "revo-grid revogr-viewport-scroll.rgCol .vertical-inner",
        '.rgCell[data-rgcol="0"]'
      );
    } catch (error) {
